...
# ponytail: torchaudio 2.9 delegates load/save to torchcodec (broken on Windows
# without ffmpeg DLLs) — route both through soundfile; patches the shared module,
# so acestep.text2music_dataset picks it up too
...
def _sf_save(uri, src, sample_rate, **kw):
    _sf.write(str(uri), src.detach().cpu().float().numpy().T, sample_rate)


torchaudio.load = _sf_load
torchaudio.save = _sf_save

# ponytail: upstream bug — get_lang references langCounts outside its try, so any
# lang-detect failure crashes the whole dataset; fall back to English
from acestep.text2music_dataset import Text2MusicDataset as _T2M

_orig_get_lang = _T2M.get_lang


def _safe_get_lang(self, text):
    try:
        return _orig_get_lang(self, text)
    except Exception:
        return "en", [], [("en", 1)]


_T2M.get_lang = _safe_get_lang
...
class Pipeline(LightningModule):
    ...
    def plot_step(self, batch, batch_idx):
        ...
        if (
            ...
            # ponytail: no process group in single-GPU mode
...
def main(args):
    ...
    # ponytail: lightning's full .ckpt is 8.5GB of model+optimizer we never resume
    # from — only the LoRA adapter (saved by on_save_checkpoint) matters. Skip the
    # giant write; it was exhausting RAM/disk and killing runs.
    class LoraOnlyCheckpoint(ModelCheckpoint):
        def _save_checkpoint(self, trainer, filepath):
            trainer.lightning_module.on_save_checkpoint({})
    ...
    trainer = Trainer(
        ...
        # ponytail: DDP needs NCCL (Linux-only); single-GPU Windows box -> auto
